    ...
    <PageBackground>
      <div className="container mx-auto px-4 py-16">
        <div className="max-w-4xl mx-auto">
          ...
          <form onSubmit={handleSearch} className="mb-8">
            <div className="flex gap-4">
              <div className="flex-1 relative">
                <Search className="absolute left-4 top-1/2 transform -translate-y-1/2 text-grey-400 h-5 w-5" />
                <input
                  type="text"
                  value={query}
                  onChange={(e) => setQuery(e.target.value)}
                  placeholder={currentLang === 'pt-br' ? 'Digite sua busca...' : 'Enter your search...'}
                  className="w-full pl-12 pr-4 py-3 border border-grey-300 rounded-lg focus:ring-2 focus:ring-grey-900 focus:border-transparent bg-white text-grey-900"
                />
              </div>
              <button
                type="submit"
                disabled={loading || !query.trim()}
                className="px-6 py-3 bg-grey-900 text-white rounded-lg hover:bg-grey-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                {loading ? (
            ...
            <div className="mt-8">
              {loading ? (
                <div className="text-center py-12">
                  <Loader2 className="h-8 w-8 animate-spin mx-auto text-grey-400 mb-4" />
                ...
                <div className="space-y-4">
                  ...
                  {results.map((result) => (
                    <Link
                      ...
                      className="block p-6 bg-white border border-grey-200 rounded-lg hover:border-grey-400 transition-colors"
                    >
          ...
          {!searched && (
            <div className="text-center py-12">
              <Search className="h-16 w-16 mx-auto text-grey-300 mb-4" />
              <p className="text-grey-600 text-lg">
                {currentLang === 'pt-br' 
